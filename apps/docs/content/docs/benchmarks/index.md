...
description: Performance measurements for all Cascade Splits instructions
sidebar:
  order: 1
---

This section contains compute unit (CU) measurements for all Cascade Splits instructions.
...
Results are written to `compute_units.md` in the `docs/benchmarks/` directory.
...
:::tip[Key insight]
Missing ATAs are **cheaper** because skipped transfers save ~6K CU each (Token CPI cost). The unclaimed storage write is much cheaper than a CPI transfer.
:::
