...
import {
  ...
  fetchSmartAccountStateByOwner as sdkFetchSmartAccountState,
  encodeTabsApiKey,
...
import { USDC_MINT, EXECUTOR_PUBKEY } from "@/lib/constants";

const USDC_DECIMALS = 6;

// =============================================================================
// Inline Helpers (moved from lib/squads.ts)
// =============================================================================

/** Fetch smart account state with app-specific constants */
async function fetchSmartAccountStateByOwner(
  rpc: Rpc<SolanaRpcApi>,
  ownerAddress: Address,
): Promise<SmartAccountState | null> {
  return sdkFetchSmartAccountState(
    rpc,
    ownerAddress,
    EXECUTOR_PUBKEY,
    USDC_MINT,
...
async function buildCreateAccountInstruction(
  rpc: Parameters<typeof fetchProgramConfig>[0],
...
function getAccountRole(acc: {
  ...
  if (typeof acc.role === "number") return acc.role;
...
/** Build instruction to withdraw USDC via executeTransactionSync */
...
async function waitForConfirmation(
  ...
  const meetsCommitment = (actual: string, required: Commitment) =>
    (COMMITMENT_LEVELS[actual] ?? -1) >= (COMMITMENT_LEVELS[required] ?? 1);
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    ...
    if (status !== null) {
      ...
      if (
        ...
        meetsCommitment(status.confirmationStatus, commitment)
      )
        return;
    }
...
export function useSmartAccount(): UseSmartAccountReturn {
  ...
  const { wallet: session } = useWalletConnection();
