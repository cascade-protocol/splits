...
import pytest
from conftest import async_w3, fund_with_usdc, get_usdc_balance
...
@pytest.fixture
async def client(anvil_fork, test_account):
...
async def test_standalone_ensure_split(anvil_fork, test_account, alice, bob) -> None:
    ...
    async with async_w3(anvil_fork) as w3:
...
async def test_execute_split_distributes_funds(anvil_fork, test_account, alice, bob) -> None:
    ...
    async with (
        ...
        assert ensure_result.split is not None
