...
export function buildInitializeProtocolInstruction(
  ...
  return new TransactionInstruction({
    programId,
    keys: [
      { pubkey: new PublicKey(protocolConfig), isSigner: false, isWritable: true },
      { pubkey: authority, isSigner: true, isWritable: true },
      { pubkey: new PublicKey(programData), isSigner: false, isWritable: false },
...
export function buildCreateSplitConfigInstruction(
  ...
  const dataSize = DISCRIMINATOR_SIZE + ADDRESS_SIZE + U32_SIZE + recipients.length * RECIPIENT_SIZE;
  ...
  const keys = [
    ...
    { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
    ...recipients.map(r => ({
      pubkey: new PublicKey(deriveAta(r.address, mint.toBase58(), tokenProgram.toBase58())),
      isSigner: false,
      isWritable: false,
    })),
  ];
...
export function buildExecuteSplitInstruction(
  ...
  const keys = [
    ...
    { pubkey: mint, isSigner: false, isWritable: false },
    { pubkey: new PublicKey(protocolConfig), isSigner: false, isWritable: false },
...
export function buildUpdateSplitConfigInstruction(
  ...
  const dataSize = DISCRIMINATOR_SIZE + U32_SIZE + newRecipients.length * RECIPIENT_SIZE;
  const data = Buffer.alloc(dataSize);
  let offset = 0;
