...
describe("messages", () => {
	describe("vaultNotEmptyMessage", () => {
		test("formats balance correctly", () => {
			const message = vaultNotEmptyMessage(1_500_000n, 6, "USDC");
			expect(message).toContain("1.5 USDC");
			expect(message).toContain("Execute the split first");
		});

		test("handles whole numbers", () => {
			const message = vaultNotEmptyMessage(5_000_000n, 6, "USDC");
			expect(message).toContain("5 USDC");
		});

		test("handles small amounts", () => {
			const message = vaultNotEmptyMessage(100n, 6, "USDC");
			expect(message).toContain("0.0001 USDC");
		});
	});

	describe("unclaimedPendingMessage", () => {
		test("singular recipient", () => {
			const message = unclaimedPendingMessage(1, 100_000n, 6, "USDC");
			expect(message).toContain("1 recipient has");
			expect(message).toContain("0.1 USDC");
		});

		test("multiple recipients", () => {
			const message = unclaimedPendingMessage(3, 500_000n, 6, "USDC");
			expect(message).toContain("3 recipients have");
			expect(message).toContain("0.5 USDC");
		});
	});
	...
	describe("skip messages", () => {
		...
		test("belowThresholdMessage", () => {
			const message = belowThresholdMessage(500_000n, 1_000_000n, 6, "USDC");
			expect(message).toContain("0.5 USDC");
