...
describe("closeSplit", () => {
	...
	test("returns CLOSED with rent recovered", async () => {
		...
		const result = await closeSplit(rpc, rpcSubscriptions, signer, mockVault);
	...
	test("returns ALREADY_CLOSED for non-existent vault", async () => {
		const rpc = createMockRpc();
		const rpcSubscriptions = createMockRpcSubscriptions();
		const signer = createMockSigner();
	...
	test("returns BLOCKED unclaimed_pending", async () => {
		const rpc = createMockRpc();
		const rpcSubscriptions = createMockRpcSubscriptions();
		const signer = createMockSigner();
	...
	test("uses rentPayer from config as rent_destination", async () => {
		...
		await closeSplit(rpc, rpcSubscriptions, signer, mockVault);
