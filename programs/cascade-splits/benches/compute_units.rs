...
use {
    helpers::{
        ...
        instructions::{
            ...
            build_close_split_config, build_initialize_protocol, build_update_protocol_config,
            build_transfer_protocol_authority, build_accept_protocol_authority,
            derive_program_data, derive_protocol_config, derive_split_config, derive_vault,
            RecipientInput, PROGRAM_ID,
        },
        serialization::{
            serialize_protocol_config, serialize_protocol_config_with_pending,
            serialize_split_config, serialize_split_config_simple,
...
fn main() {
    ...
    let (exec_max_ix, exec_max_accounts) = {
        ...
        let recipient_atas: Vec<Pubkey> = recipient_keys.iter().map(|r| derive_ata(r, &mint)).collect();
        let protocol_ata = derive_ata(&fee_wallet, &mint);

        let recipients: Vec<RecipientData> = recipient_keys.iter().map(|&addr| {
            RecipientData { address: addr, percentage_bps: 495 }
        }).collect();
    ...
    let (exec_unclaimed_1_of_2_ix, exec_unclaimed_1_of_2_accounts) = {
        ...
        let recipients = vec![
            RecipientData { address: recipient1, percentage_bps: 5000 },
            RecipientData { address: recipient2, percentage_bps: 4900 },
        ];
    ...
    let (create_multi_ix, create_multi_accounts) = {
        ...
        let recipients = vec![
            RecipientInput { address: recipient1, percentage_bps: 3000 },
            RecipientInput { address: recipient2, percentage_bps: 2500 },
            RecipientInput { address: recipient3, percentage_bps: 2000 },
            RecipientInput { address: recipient4, percentage_bps: 1500 },
            RecipientInput { address: recipient5, percentage_bps: 900 },
        ];
    ...
    let (update_multi_ix, update_multi_accounts) = {
        ...
        let new_recipient_atas: Vec<Pubkey> = new_recipient_keys.iter().map(|r| derive_ata(r, &mint)).collect();

        let new_recipients: Vec<RecipientInput> = new_recipient_keys.iter().map(|&addr| {
            RecipientInput { address: addr, percentage_bps: 990 }
        }).collect();
    ...
    MolluskComputeUnitBencher::new(mollusk)
        ...
        .bench(("update_protocol_config", &update_protocol_ix, &update_protocol_accounts))
        .bench(("transfer_protocol_authority", &transfer_auth_ix, &transfer_auth_accounts))
        .bench(("accept_protocol_authority", &accept_auth_ix, &accept_auth_accounts))
        // Split config lifecycle
        .bench(("create_split_config_1_recipient", &create_ix, &create_accounts))
        .bench(("create_split_config_5_recipients", &create_multi_ix, &create_multi_accounts))
        .bench(("update_split_config_to_2", &update_ix, &update_accounts))
        .bench(("update_split_config_to_10", &update_multi_ix, &update_multi_accounts))
        ...
        .bench(("execute_split_20_recipients", &exec_max_ix, &exec_max_accounts))
        // Execute split - unclaimed scenarios
        .bench(("execute_split_unclaimed_1_of_2", &exec_unclaimed_1_of_2_ix, &exec_unclaimed_1_of_2_accounts))
        .bench(("execute_split_unclaimed_1_of_5", &exec_unclaimed_1_of_5_ix, &exec_unclaimed_1_of_5_accounts))
        .bench(("execute_split_unclaimed_4_of_5", &exec_unclaimed_4_of_5_ix, &exec_unclaimed_4_of_5_accounts))
        .bench(("execute_split_unclaimed_5_of_5", &exec_unclaimed_5_of_5_ix, &exec_unclaimed_5_of_5_accounts))
        .must_pass(true)
