...
const FEE_WALLET = new PublicKey('2zMEvEkyQKTRjiGkwYPXjPsJUp8eR1rVjoYQ7PzVVZnP');
...
async function main() {
  ...
  const protocolConfig = await connection.getAccountInfo(new PublicKey(protocolConfigAddress));
  ...
  console.log('\nSending transaction...');
  const signature = await sendAndConfirmTransaction(connection, transaction, [deployerKeypair]);
